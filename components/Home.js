...
function Home() {
  ...
  useEffect(() => {
    fetch("https://mymoviz-part5-backend-zeta.vercel.app/movies")
      ...
      .then((data) => {
        ...
        const moviesData = data.movies.map((movie) => ({
          title: movie.title,
          poster: `${posterSecureBaseUrl}${posterSize}${movie.poster_path}`,
          oteAverage: data.movies[0].vote_average,
  ...
  const moviesList = moviesData.map((data, i) => {
